...
Title: The part of MQTT data transfer program on dronekit python
...
class MqttCtrl(ardctrl.ArdCtrlClsC2):
    ...
    def on_message(self, client, userdata, msg):
        ...
        if msg.topic==self.topic_dctrl:
            ...
            # Drone mode 
            elif self.drone_command["operation"] == "GUIDED":
                dlog.LOG("DEBUG", "GUIDED")
                self.set_vehicle_mode("GUIDED")
            elif self.drone_command["operation"] == "AUTO":
                dlog.LOG("DEBUG", "Set mode to AUTO")
                self.set_vehicle_mode("AUTO")
            elif self.drone_command["operation"] == "RTL":
                dlog.LOG("DEBUG", "Set mode to AUTO")
                self.set_vehicle_mode("RTL")
            # Arm/Disarm
            ...
            # Land
            elif self.drone_command["operation"] == "LAND":
                dlog.LOG("DEBUG", "Landing")
                self.set_vehicle_mode("LAND")
            # Pause
