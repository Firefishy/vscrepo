...
var WP_list=[//0番はサンプル 
  [["lng","lat"],"WP_speed","WP_altitude",["WPaction([actionNo,parameter])"]],
...
function open_point_tab(id,type){
    open_point=[type,id]
	$("#add_WPaction_button").prop('disabled', false); //アクションの追加制限を解除 必要な場合add_actionで再度制限される
    ...
    if(type=='waypoint'){//ウェイポイントがクリックされたとき
        ...
        for(let i=0; i<WP_list[id][3].length; i++){
            add_action(WP_list[id][3][i][0],WP_list[id][3][i][1])
        }
    }
...
function close_point_tab(delete_flg){
	...
	if(delete_flg!=true){
		if(open_point[0]=='waypoint'){
			let action_list=[]
			$('#columns li#WP_action').each(function() {
				let WP_action_detail=[$(this).attr("value")]
				if($(this).find(".action_input").length){
					WP_action_detail.push($(".action_input").val());
				}
				//console.log(WP_action_detail)
				action_list.push(WP_action_detail);
...
//==================WPアクション ここから==================
//ウェイポイントアクションの追加
function change_addWPaction(val){
	let text = WPaction_name[String(val)]
	$("#add_WPaction_button").text(text)
	$("#add_WPaction_button").data('val',val)
}
function add_action(value,parameter){
	if(value==undefined){
		value = $("#add_WPaction_button").data('val')
	}
	if(parameter==undefined){
		parameter=0; //WPアクション内の設定値の初期値 0だとまずいもの（高度設定とか？）が追加されたらWPの値などからとるように変える？
	}
	
	
	let add_column="<li class='column' id='WP_action' value="+value+"><details id='actions'><summary draggable='true' >"
		+WPaction_name[value]+"</summary>"
		+"<div class='action_contents'><div class='flt_op_sidebar_seg mb-3'></div>"
	if(value==1){//value=1 ホバリングの時
		add_column=add_column
			+"<div>"
			+"<span>ホバリング時間</span><br>"
			+"<span class='number_input_action'>"
			+"<input type='number' class='action_input w-75' id='action_hovering_time' min=0 max=300 value="+ parameter +">"
			+"<span>秒</span></span>"+"<br><br>"
			//+"<input type='range' id='action_hovering_time' class='form-control slider' min='0' max='300' step='1' value="+parameter+">"
			+"</div><div class='flt_op_sidebar_seg mb-3'></div>"
	}else if(value==5){//value=5 機体回転角度設定の時
		add_column=add_column
			+"<div>"
			+"<span>機体回転角度設定</span><br>"
			+"<span class='number_input_action'>"
			+"<input type='number' class='action_input w-75' id='action_aircraft_direction' min=0 max=360 value="+ parameter +">"
			+"<span>度</span></span>"
			+"<input type='range' id='action_aircraft_direction' class='form-control slider' min='0' max='360' step='0.1' value="+parameter+">"
			+"</div><div class='flt_op_sidebar_seg mb-3'></div>"
	}/*else if(value==6){//value=6 ジンバルピッチ設定の時
		add_column=add_column
			+"<div>"
			+"<span>ジンバルピッチ設定</span><br>"
			+"<span class='number_input_action'>"
			+"<input type='number' class='action_input w-75' id='action_camera_direction_txt' min=0 max=90 value="+ parameter +">"
			+"<span>度</span></span>"
			+"<input type='range' id='action_camera_direction' class='form-control slider' min='0' max='90' step='0.1' value=" + parameter+"'>"
			+"</div><div class='flt_op_sidebar_seg mb-3'></div>"
	}*/
	//削除ボタンの追加
	add_column=add_column+"<div class='row justify-content-center my-2'>"
		+"<button type='button'class='btn btn-danger' id='remove_action_button' >"+
		"アクションの削除</button></div></div>"
	add_column=add_column+"</details></li>"
	$("ul li#blank_object").before(add_column)
	PageLoad();
	//WPアクションの数で追加の制限、制限解除は削除に組み込んだ
	if($("ul li#WP_action").length>=8){
		$("#add_WPaction_button").prop('disabled', true);
	}
}
//WPアクション内部の数値入力の更新
$(function(){
	$('#columns').on('click','#remove_action_button',function(){
		$(this).parent('div').parent('div').parent('details').parent('li').remove()
		$("#add_WPaction_button").prop('disabled', false);
	})
	$('#columns').on('change','.action_input',function(){
		change_action_input(this)
	})
	$('#columns').on('change input','.slider',function(){
		change_action_slider(this)
	})
	function change_action_input(_this){
		let resultFeild = $(_this).parent($(".numberinput_unit")).next(".slider");
		...
		$(this).val(value)
		resultFeild.val(value);
		change_action_slider(resultFeild);
	}
	
	function change_action_slider(_this){
		let resultFeild = $(_this).prev($(".numberinput_unit")).children(".action_input");
		let value = Number($(_this).val());
		resultFeild.val(value);
		$(_this).attr('value',value)
		resultFeild.attr('value',value)
	}
})
//ドラッグアンドドロップ https://www.ipentec.com/document/javascript-list-sortable-item
var dragSrcEl = null;
...
//	"4":"ビデオ録画停止",
	"5":"aircraft_rotate",
//	"6":"ジンバルピッチ設定"
};//ウェイポイントアクションの名前(メッセージ送信用)
const WPaction_data_name = {
	"1":"hovering_time",
//	"2":"写真撮影",
//	"3":"ビデオ録画開始",
//	"4":"ビデオ録画停止",
...
function save_event(){
	close_point_tab(false); //ポイントタブを閉じる+ポイントのデータを保存する関数を呼び出し
	const WP_list_copy = Array.from(WP_list);
	saved_WP_list = []
	// WPごとに [[ 緯度, 経度 ], 高度 , 速度 , アクション([No,para])]のリストにしてまとめる 
...
////////////////////////////////////////////////////////////////////////////////////////////////
	...
	const makeMissionCmdWithTab = (
		...
		cmd,              // command: 16(WPT), 22(TOFF), 20(RTL), 21(LAND)
		lat , lon, alt,
		cnt	
	) => {
			missionData =   idx + '\t' 
...
////////////////////////////////////////////////////////////////////////////////////////////////
	...
	saved_WP_num=saved_WP_list.length; //WPの数の確認
	let cmdAry = [];
	let mission_cmd;

	var toJSON_action_list = { //ウェイポイントアクション用のデータ
		"WP_num":saved_WP_num,
	}
	...
	for(let j=0; j<saved_WP_num; j++){
		...
		mission_cmd = makeMissionCmdWithTab(
			...
			3,
			16, //16:WPT 
			saved_WP_list[j][0][0],//lat
			saved_WP_list[j][0][1],//lon
			saved_WP_list[j][1],//alt
			1
		);
		...
		/*  //ルートデータメッセージの作成　旧版
		let WP_data={
			"frame":3,       //固定値、高度の取り方（対地高度）
			"command":16,    //固定値、
			"d_lat":saved_WP_list[j][0][0],
			"d_lon":saved_WP_list[j][0][1],
			"d_alt":saved_WP_list[j][1],
			"d_speed":Number($('#speed').val())  //saved_WP_list[j][2]
		}
		toJSON_WP_list["WP_data"+(j+1)]=WP_data
		//console.log(toJSON_WP_list)
		*/

		//////////////////////////////////////////////////////////////////////
		//WPアクションメッセージの作成
		let WPaction_num=saved_WP_list[j][3].length; //アクションの個数
		toJSON_action_list["WP"+(j)+"_actions"]={  //WPごとの項目を追加
			"waypoint": j,                         //対応するWPの番号
			"action_num" : WPaction_num,             //このWPのアクションの個数
		}
		if(WPaction_num>0){                          //アクションがあるときアクションごとに項目追加
			for (let k=0; k<WPaction_num; k++){
				let action_data={
					"action_type" : WPaction_type[saved_WP_list[j][3][k][0]], //アクションタイプ
				}
				if(saved_WP_list[j][3][k][1]){//アクションのデータがあるなら追加（写真撮影などデータがないものを想定）
					//キー：hovering_timeなどアクションごとに名前が変わる  バリュー：今の形だと値が複数だとリストが入ることになる
					action_data[WPaction_data_name[saved_WP_list[j][3][k][0]]]=saved_WP_list[j][3][k][1];
				}
				toJSON_action_list["WP"+(j)+"_actions"]["WP"+(j)+"_action"+(k)]=action_data;
			}
		}
		///////////////////////////////////////////////////////////////////////////////
	}
	...
	//ミッション終了時動作を追加
	//ミッション完了時のアクション  画面上の選択肢:入力される値={ホーム帰還(最短距離):"1",(ホーム帰還(ルート沿い):"4"), その場で着陸:"3", (ホバリング:"2")  } 
	if($('#mission_complete').val()=="1"){ //
		mission_cmd = makeMissionCmdWithTab(idx, 0, 3, 20, clatData, clonData, caltData, 1); 
//	}else if($('#mission_complete').val()=="4"){ //smartRTL
//		mission_cmd = makeMissionCmdWithTab(idx, 0, 3, ?, clatData, clonData, caltData, 1); 
	}else if($('#mission_complete').val()=="3"){ //LAND
		mission_cmd = makeMissionCmdWithTab(idx, 0, 3, 21,
			saved_WP_list[saved_WP_num][0][0], //lat
			saved_WP_list[saved_WP_num][0][1], //lon
			saved_WP_list[saved_WP_num][1], 1); //alt, cnt
	}
	cmdAry.push(mission_cmd)  
...
console.log(mission_cmd);
cmdAry.push(mission_cmd);
	console.log(toJSON_action_list)
	console.log(toJSON_WP_list)
	let json_WP_data = JSON.stringify(toJSON_WP_list);     // JSON型にする
	let JSON_action_data = JSON.stringify(toJSON_action_list);
	
	send_message(json_WP_data);      //MQTTでルートデータをpublish
	send_message(JSON_action_data);  //MQTTでアクションデータをpublish
	
	pagechange(WP_list_copy,$('#mission_complete').val(), $('#interruption').val()) //画面遷移とデータの受け渡し
}
